...
$('askAiBtn').onclick = async()=>{ $('aiAnswer').textContent='NexIA réfléchit...'; const r=await call('ai.php',{prompt:$('aiPrompt').value, conversation_id: state.conversationId}); $('aiAnswer').textContent=r.reply; };
$('aiAnswer').ondblclick = async()=>{ if(!state.conversationId || !$('aiAnswer').textContent.trim()) return; await call('messages.php?action=send',{conversation_id:state.conversationId,body:$('aiAnswer').textContent,message_type:'ai',client_id:crypto.randomUUID()}); await load(); };
